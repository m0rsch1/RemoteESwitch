...
uint8_t WaspSX1272::ON()
{
	...
	USB.println(F("INCLUDES V26 FIXES & ESA FIXES!"));
...
byte WaspSX1272::readRegister(byte address)
{
    ...
    SPI.setSPISlave(ALL_DESELECTED); // V26FIX
...
uint8_t	WaspSX1272::getHeader()
{
	...
	if( bitRead(readRegister(REG_MODEM_CONFIG1), 2) == 0 ) // V26FIX
	...
	{ 
		...
		else
		{
			...
			{ 
				...
				//writeRegister(REG_OP_MODE, LORA_STANDBY_MODE);	// ESAFIX: Prevent standby mode like below (timeout handling)
			}
			else
			{ 
...
int8_t WaspSX1272::getPacket(uint32_t wait)
{
	...
	{		
		...
		if( _modem == LORA )
		{
			_payloadlength = packet_received.length  > OFFSET_PAYLOADLENGTH ? packet_received.length - OFFSET_PAYLOADLENGTH : 0; // ESAFIX: Prevent underflow
		}
